...
FetchWebDev() {
  curl -L https://codeload.github.com/GitCourser/web-dist/tar.gz/refs/heads/dev -o web-dist-dev.tar.gz
...
FetchWebRelease() {
  curl -L https://github.com/GitCourser/alist-web/releases/latest/download/dist.tar.gz -o dist.tar.gz
...
BuildRelease() {
  ...
  xgo -targets=windows/amd64,linux/amd64,linux/arm64 -out "$appName" -trimpath -ldflags="$ldflags" -tags=jsoniter .
...
BuildReleaseLinuxMusl() {
  ...
  OS_ARCHES=(linux-musl-amd64 linux-musl-arm64 linux-musl-mipsle)
  CGO_ARGS=(x86_64-linux-musl-gcc aarch64-linux-musl-gcc mipsel-linux-musl-gcc)
  for i in "${!OS_ARCHES[@]}"; do
    os_arch=${OS_ARCHES[$i]}
    cgo_cc=${CGO_ARGS[$i]}
    echo building for ${os_arch}
    ...
    export CC=${cgo_cc}
    export CGO_ENABLED=1
    if [[ "$os_arch" == *"mipsle"* ]]; then
      export GOMIPS=softfloat
    else
      unset GOMIPS
    fi
    go build -o ./build/$appName-$os_arch -trimpath -ldflags="$muslflags" -tags=jsoniter .
  done
}
