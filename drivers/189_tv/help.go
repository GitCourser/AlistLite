...
var urlPathRegexp = regexp.MustCompile(`://[^/]+((/[^/\s?#]+)*)`)

func extractURLPath(fullUrl string) string {
	matches := urlPathRegexp.FindStringSubmatch(fullUrl)
	if len(matches) > 1 {
		return matches[1]
	}
	return ""
...
func signatureOfHmac(sessionSecret, key, operate, fullUrl, extra string) string {
	urlpath := extractURLPath(fullUrl)
	mac := hmac.New(sha1.New, []byte(sessionSecret))
	data := fmt.Sprintf("%s&Operate=%s&RequestURI=%s&%s", key, operate, urlpath, extra)
	mac.Write([]byte(data))
...
func SessionKeySignatureOfHmac(sessionSecret, sessionKey, operate, fullUrl, dateOfGmt string) string {
	return signatureOfHmac(sessionSecret, "SessionKey="+sessionKey, operate, fullUrl, "Date="+dateOfGmt)
}
...
func AppKeySignatureOfHmac(sessionSecret, appKey, operate, fullUrl string, timestamp int64) string {
	return signatureOfHmac(sessionSecret, "AppKey="+appKey, operate, fullUrl, "Timestamp="+fmt.Sprint(timestamp))
}
